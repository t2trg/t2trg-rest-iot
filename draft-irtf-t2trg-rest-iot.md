...
CoAP responses with code 2.05, 4.xx, and 5.xx are cacheable, whereas responses with code 2.03 update the cache lifetime of an existing cached response (see {{Section 5.9.1.3 of RFC7252}}).
...
With transport layer security, the security association terminates at every intermediary, so an intermediary that is not trusted with the content cannot be part of the path at all.
For IoT systems, object security {{RFC8613}} may therefore be preferable, as it protects the message end-to-end and lets an untrusted intermediary forward the response.
Such a response can also be cached at the intermediary and served to several clients, using the mechanism specified in {{I-D.ietf-core-cacheable-oscore}}.
...
In HTTP, there exist a number of workarounds to enable server push, e.g., long polling and streaming {{RFC6202}} or server-sent events (SSE) {{W3C.REC-html5-20141028}}.
...
Streaming and SSE (the latter is actually an evolution of the former) are more efficient, as only one request is sent.
...
Consequently, the notifications are not self-describing representations, and hence not RESTful.
